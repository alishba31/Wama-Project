...
//   try {
//     const { troubleTicketId, newStatus } = await request.json();
...
//     // Update the trouble ticket status in the database
...
//       `Status updated to: ${updatedTicket.status}`
//     );

//     return NextResponse.json(
//       {
//         message: "Trouble ticket status updated and notification sent",
...
//       console.error("Error updating trouble ticket status:", error.message);
//       return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedTicket = await prisma.troubleTicket.update({
      where: { id: troubleTicketIdInt },
      data: { adminStatus: newStatus, remarks },
      include: {
        User: true,
        FormSubmission: true,
      },
    ...
    await sendNotification(
      updatedTicket.User.email,
      `Trouble Ticket: ${updatedTicket.title}`,
      `Status updated to: ${updatedTicket.adminStatus}`
    );
